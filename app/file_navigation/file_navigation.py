...
def build_dir(suffix, path='/'):
    if (type(suffix) != str or type(path) != str):
        raise TypeError("Expected one or two strings but received an object of a different type")
...
def filter_out_hidden_files(files):
    ...
    if not (type(files) == list):
        raise TypeError("Input should be a list, but an object of type {} was received.".format(type(files)))
...
def get_files(ignore_hidden_files=True, path=None):
